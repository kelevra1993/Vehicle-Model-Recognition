...
with open('final_trainer.csv','r') as csvfile:
	...
	for row in linereader :
		...
		if((os.path.exists(root+"/"+make+"/"+image_names[0]+"_2.tiff"))):
			...
			image_3=cv2.imread(root+"/"+make+"/"+image_names[0]+"_2.tiff")
			image_4=cv2.imread(root+"/"+make+"/"+image_names[0]+"_2.tiff")
			
			# upper_left[0]=int(firsts[0])-2*delta_x
			upper_left[0]=int(firsts[0])-int(0.5*delta_x)
			...
			lower_right[0]=int(seconds[0])+int(0.5*delta_x)
			...
			sift2=cv2.xfeatures2d.SIFT_create(edgeThreshold=40,contrastThreshold=0.04)
			
			# print(inspect.signature(cv2.xfeatures2d.SIFT_create))
			
			kp=sift.detect(image_2,None)
			kp2=sift2.detect(image_2,None)
			print("length of default keypoints is", len(kp))
			print("length of second keypoints is", len(kp2))
			print("\n")
			...
			for i in range(len(kp2)):
				if((kp2[i].pt[0]<upper_left[0] or kp2[i].pt[0]>lower_right[0] or kp2[i].pt[1]<full_upper_left[1] or kp2[i].pt[1]>full_lower_right[1]) 
				or (kp2[i].pt[0]>plate_upper_left[0] and kp2[i].pt[0]<plate_lower_right[0] and kp2[i].pt[1]>plate_upper_left[1] and kp2[i].pt[1]<plate_lower_right[1]) or kp2[i].pt[1]>banner_height):
					undesired_keypoints2.append(i)
					
			for i in reversed(undesired_keypoints2):
				kp2.pop(i)
			
			image_3=cv2.drawKeypoints(image_3,kp,image_3)
			image_4=cv2.drawKeypoints(image_4,kp2,image_4)
			...
			counter+=1
			sh=image_2.shape
			image_3=cv2.rectangle(image_3,(upper_left[0],full_upper_left[1]),(lower_right[0],full_lower_right[1]),(0,255,0),8)
			image_4=cv2.rectangle(image_4,(upper_left[0],full_upper_left[1]),(lower_right[0],full_lower_right[1]),(0,255,0),8)
			
			if(image_shape[0]==3000 or image_shape[0]==2136):
				image_3=cv2.resize(image_3,(int(sh[1]/3),int(sh[0]/3)))
				image_4=cv2.resize(image_4,(int(sh[1]/3),int(sh[0]/3)))
			else:
				image_3=cv2.resize(image_3,(int(sh[1]/2),int(sh[0]/2)))
				image_4=cv2.resize(image_4,(int(sh[1]/2),int(sh[0]/2)))
			# print(image_shape)
			# print(image_names[0]+"_2.tiff"+" : "+str(len(kp)))

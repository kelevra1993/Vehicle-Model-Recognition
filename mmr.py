...
final_path=["aston","bmw","clio","dodge","peugot","cliotranslate"]
# paths=["aston","bmw","clio","dodge","peugot","cliotranslate"]
...
if(num_npy!=num_images or compute_all_steps):
	...
	fun.compute_save_reduced_root_sift(reducer,final_path)
...
for gmm_comp in range(50,1000,50):
	...
	if(os.path.isfile(gmm_means_file) and os.path.isfile(gmm_covariance_file) and os.path.isfile(gmm_weight_file)):
		...
		#CASE TESTING FOR FULL AND DIAGONAL COVARIANCES TYPES
		GMM=gauss.GaussianMixture(n_components=gmm_comp,covariance_type="diag",max_iter=10000,n_init=1,init_params="kmeans")
		GMM.fit(descriptors)
	...
	if(compute_all_steps):
		...
		fun.generate_fisher_vectors(final_path,means,covs,weights,"_"+str(gmm_comp))
...
for gmm_comp in range(50,1000,50):
	...
	print(cosine_metric.shape)
	# for i in range(10):
		# print("diagonal is :",cosine_metric[i,i])
		# print(cosine_metric[i])
		# print("\n")
	all_bmw=0		
	all_clio=0			
	...
	all_peugot=0				
	all_aston=0	
	# all_clio_translate=0
	#for translated clios
	# for ind in range(105,115):
	for ind in range(65,75):
		# print(cosine_metric[ind])
		# print(np.flip(np.sort(cosine_metric[ind]),axis=0))
		indices=np.flip(np.argsort(cosine_metric[ind]),axis=0)
		# print(indices)
		aston=0
		bmw=0
		...
		for  sim in range(1,(evaluation_limit+1)):
			...
			if(indices[sim]>79 and indices[sim]<100):
				peugot=peugot+1
			# if(indices[sim]>99):
				# clio_translate=clio_translate+1
	...
	# print("TRANSLATED CLIOS : %d"%all_clio_translate)
	current=all_dodge
